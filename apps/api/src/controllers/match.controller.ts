...
export const MatchController = {
  ...
  buscarPorId: async (req: Request, res: Response) => {
    try {
      ...
      if (!id) return res.status(400).json({ error: 'Id do match invÃ¡lido.' })
  ...
  listarPorEstagiario: async (req: Request, res: Response) => {
    try {
      ...
      if (!estagiarioId) return res.status(400).json({ error: 'Id do estagiÃ¡rio invÃ¡lido.' })

      if (req.auth?.role === 'estagiario' && req.auth.id_estagiario !== estagiarioId) {
        return res.status(403).json({ error: 'VocÃª nÃ£o pode ver matches de outro estagiÃ¡rio.' })
      }
      ...
      console.error(error)
      return res.status(500).json({ error: 'Erro ao listar matches do estagiÃ¡rio.' })
    }
  },
  ...
  listarPorVaga: async (req: Request, res: Response) => {
    try {
      ...
      if (!vagaId) return res.status(400).json({ error: 'Id da vaga invÃ¡lido.' })
      ...
      if (req.auth?.role === 'empresa' && req.auth.id_empresa !== vaga.id_empresa_empresa) {
        return res.status(403).json({ error: 'VocÃª nÃ£o pode ver candidatos de outra empresa.' })
      }
  ...
  criar: async (req: Request, res: Response) => {
    try {
      ...
      if (!idEstagiario || !idVaga) {
        return res.status(400).json({ error: 'Informe estagiÃ¡rio e vaga.' })
      }

      if (req.auth?.id_estagiario !== idEstagiario) {
        return res.status(403).json({ error: 'VocÃª nÃ£o pode criar candidatura para outro estagiÃ¡rio.' })
      }
      ...
      if (!estagiario) return res.status(404).json({ error: 'EstagiÃ¡rio nÃ£o encontrado.' })
      ...
      if (candidaturaExistente) {
        return res.status(409).json({ error: 'VocÃª jÃ¡ curtiu/se candidatou a esta vaga.' })
      }
  ...
  atualizarStatus: async (req: Request, res: Response) => {
    try {
      ...
      if (req.auth?.id_empresa !== matchAtual.vaga.id_empresa_empresa) {
        return res.status(403).json({ error: 'VocÃª nÃ£o pode alterar candidatos de outra empresa.' })
      }
  ...
  deletar: async (req: Request, res: Response) => {
    try {
      ...
      if (!podeRemover) {
        return res.status(403).json({ error: 'VocÃª nÃ£o pode remover este match.' })
      }
